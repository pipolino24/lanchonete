...
export function ProductModal({
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${productId}`)
      ...
      .then((d) => {
        if (active && d && Array.isArray(d.complementLinks)) {
          setDetail(d);
          setLoading(false);
        } else if (active) {
          setLoading(false);
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="relative z-10 flex max-h-[92vh] w-full max-w-lg animate-slide-up flex-col overflow-hidden rounded-t-3xl border border-coal-700 bg-coal-900 sm:rounded-3xl">
        ...
        {loading ? (
          ...
          <div className="grid h-72 place-items-center gap-3 px-6 text-center">
            <p className="text-sm text-ash">Produto indisponível no momento.</p>
            <button onClick={onClose} className="rounded-lg bg-coal-800 px-4 py-2 text-sm text-cream ring-1 ring-coal-700">
              Fechar
            </button>
          </div>
        ) : (
